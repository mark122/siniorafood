...
class QuizQuestionController extends Controller
{
    ...
    public function fetchAvailableQuestions($id, QuestionFilters $filters)
    {
        ...
        if($type->name === 'Personality') {
            $questions = Question::filter($filters)->whereNotIn('id', $quiz->questions->pluck('id'))
            ->where('question_type_id',9)->where('is_active',1)
            ->with(['questionType:id,name,code', 'difficultyLevel:id,name,code', 'skill:id,name'])
            ->paginate(10);
        } else {
            $questions = Question::filter($filters)->whereNotIn('id', $quiz->questions->pluck('id'))
    ...
    public function AddAllFilteredAvailableQuestions($id, QuestionFilters $filters)
    {
        $quiz = Quiz::select(['id', 'title','quiz_type_id'])->with(['questions' => function ($builder) {
            $builder->select('id');
        }])->findOrFail($id);
        ...
        $type = $quiz->quizType;
        $questions = [];
        $questions = Question::filter($filters)->whereNotIn('id', $quiz->questions->pluck('id'))->where('question_type_id','!=',9)->where('is_active',1)->with(['questionType:id,name,code', 'difficultyLevel:id,name,code', 'skill:id,name'])->get()->pluck('id');

        foreach ($questions as $key => $value) {
            $quiz2 = Quiz::select(['id', 'title','quiz_type_id', 'settings'])->findOrFail($id);
            $question = Question::with('questionType:id,type')->findOrFail($value);
            ...
            if (!$quiz2->questions->contains($question->id)) {
                $quiz2->questions()->attach($question->id, ['quiz_id' => $quiz2->id]);
            }
        }
        $quiz2->updateMeta_type_quiz();
    ...
    public function removeAllQuestion($id)
    {   
        ...
        foreach ($dd as $key => $value) {
            $question = Question::with('questionType:id,type')->findOrFail($value->id);
            $quiz = Quiz::select(['id', 'title','quiz_type_id', 'settings'])->findOrFail($id);
            $quiz->questions()->detach($question->id);
        }

        $quiz->updateMeta_type_quiz();
