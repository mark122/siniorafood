...
class UserQuizRepository
{
    ...
    public function createSession(Quiz $quiz, QuestionRepository $questionRepository)
    {
        ...
        if ($quiz->quiz_type_id == 1) {
            $questions = $quiz->questions()->with(['questionType:id,name,code'])->get()->groupBy('skill_id');
            // dd($questions);
            $questioncollection = collect();
            $i = 0;
            foreach ($questions as $key => $quesgroup) {
                // dd($quesgroup);
                $randomItems = $quesgroup->random(4);
                foreach ($randomItems as $key => $value) {
                    $questioncollection->put($i, $value);
                    $i++;
                }
            }
    ...
    public function getInstructions(Quiz $quiz)
    {
        ...
        return [
            ...
            "standard" => [
                ...
                "If for any reason you were logged out during the assessment, you will be able to continue where you stopped by navigating to the <strong>My Progress</strong> tab from the Dashboard."
            ]
        ];
    }
