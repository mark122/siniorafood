...
class Quiz extends Model
{
    ...
    */
    ...
    public function updateMeta_type_quiz()
    {
        $this->total_questions = $this->questions()->count()/2;
        $this->total_questions = intval($this->total_questions);

        if ($this->settings->get('auto_duration', true)) {
           $this->total_duration = $this->questions()->sum('default_time')/2;
        }

        if ($this->settings->get('auto_grading', true)) {
            $this->total_marks = $this->questions()->sum('default_marks')/2;
            $this->total_marks = intval($this->total_marks);
        } else {
            $this->total_marks = $this->questions()->count() * ($this->settings->get('correct_marks')/2);
            $this->total_marks = intval($this->total_marks);
        }
