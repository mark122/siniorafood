<template>
    <admin-layout>
        ...
        <template #actions>
            ...
            <button @click="ShowmultiPartResult(quiz.id)" v-if="quiz.quiz_type_id == 1" class="qt-btn qt-btn-success" type="button">Graph</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ShowmultiPartResult(id) {
                this.$confirm.require({
                    header: 'Confirm',
                    message: 'Do you want to view both parts results ?',
                    //icon: 'pi pi-info-circle',
                    rejectClass: 'p-button-danger',
                    acceptClass: 'p-button-danger',
                    rejectLabel: 'Yes both parts',
                    acceptLabel: 'No only this part',
                    accept: () => {
                        this.$inertia.get(route('graph_quizzes.detailed_report', { quiz: this.quiz.id }));
                    },
                    reject: () => {
                        this.$inertia.get(route('graph_quizzes.overall_report', { quiz: this.quiz.id }));
                    }
                });
            }
        },
