<template>
    <app-layout>
        <template #header>
            <h4 class="page-heading">{{ __('Assessments') }}</h4>
        </template>
                                ...
                                <quiz-schedule-card :quiz="quiz">
                                    <template #action>
                                        <inertia-link :href="route('quiz_schedule_instructions', {quiz: quiz.slug, schedule: quiz.code})">
                                            <span class="qt-btn qt-btn-sm qt-btn-success">View Instructions</span>
                                        </inertia-link>
                                    </template>
                                </quiz-schedule-card>
                            </template>
...
<script>
    ...
    export default {
        ...
        computed: {
            title() {
                return 'Assessment Dashboard - ' + this.$page.props.general.app_name;
            }
        },
    }
</script>
